...
namespace Inventory_Management_System
{
    public partial class Add_Product : Form
    {
        ...
        public Add_Product()
        {
            ...
            candidatePrtsGrid.DataSource = topTable;

            var botTable = new BindingSource();
            botTable.DataSource = addedParts;
            assoPrtsGrid.DataSource = botTable;
        }
        ...
        private void DeleteAssociatedPartButton_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes) {
                ...
                foreach (DataGridViewRow row in assoPrtsGrid.SelectedRows)
                {
                    assoPrtsGrid.Rows.RemoveAt(row.Index);
                }
            }
            else return;
        ...
        {
            int partID = int.Parse(srchPrtTxtBox.Text);
            Part match = Inventory.LookupPart(partID);
            foreach (DataGridViewRow row in candidatePrtsGrid.Rows)
            {
        ...
        {
            Part partToAdd = (Part)candidatePrtsGrid.CurrentRow.DataBoundItem;
        ...
        private void SaveNewProductButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                min = int.Parse(addPrdctMinTxtBox.Text);
                max = int.Parse(addPrdctMaxTxtBox.Text);
                inventory = int.Parse(addProductInventoryTextBox.Text);
                price = decimal.Parse(addPrdctPriceTxtBox.Text);
            }
            catch
            {
